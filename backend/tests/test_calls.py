...
async def run():
    ...
    body = _payload(
        [{"id": "tc1", "name": "resolve_item", "arguments": {"mention": "chips"}}]
    )
        ...
        [
            {
                "id": "tcA",
                "name": "resolve_item",
                "arguments": {"mention": "Doritos chips"},
            },
            {
                "id": "tcB",
                "name": "resolve_brand",
    ...
    body = {
        "message": {
            ...
            "call": {"id": "c2"},
            "toolCallList": [
                {"id": "tcX", "name": "resolve_item", "arguments": {"mention": "chips"}}
            ],
        }
    }
    resp = await calls.vapi_webhook(_FakeRequest(body), db)
